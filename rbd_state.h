...
namespace rbd {
    ...
    struct Position : public Eigen::Matrix<Scalar, 7 + JointSpaceDims, 1, Eigen::DontAlign> {
        ...
        struct Velocity : public Eigen::Matrix<Scalar, velocity_dims, 1, Eigen::DontAlign> {
            ...
            inline auto base_lie() const {
                return Eigen::AngleAxis<Scalar>(base_angular().norm(), Traits<Scalar>::normalize(base_angular()));
            }
        };
        ...
        auto& operator+=(const Velocity& velocity) {
            ...
            base_position() += base_rotation() * velocity.base_linear();

            auto rotation = base_rotation() * velocity.base_lie();
    ...
    public:
        using Base = Eigen::Matrix<Scalar, 13 + 2 * JointSpaceDims, 1>;
